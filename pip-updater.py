...
"--add-file" option to add packages to exceptions.txt via command line.""")
    ...
    group.add_argument('-c', '--crontab', required=False, action='store', metavar='cron', nargs='+',
    ...
    group.add_argument('-a', '--add-pkgs', required=False, action='store', metavar='file', nargs='+',
...
def get_pip_packages(config, local_path):
    ...
    outdated_object = subprocess.run('pip list --outdated --format=json', shell=True, capture_output=True, text=True)
        ...
        for element in outdated_data:
            ...
            if pkg_name in exceptions_data:
                if exceptions_data[pkg_name] is None:
                    ...
                    msg = f'{pkg_name} freezed at {exceptions_data[pkg_name]} - exception noted at exceptions.txt'
                    print(msg)
                    logging.info(msg)
                    update_single_package(pkg_name, pkg_version, exceptions_data[pkg_name], exceptions)
                    continue
            else:
                if interactive:
                    while True:
